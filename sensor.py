...
class ConcatWrapper:
    ...
    def is_uninit(self):
        if(self.x1 is None and self.x2 is None and self.x3 is None and self.drop4 is None):
            return True
        else:
            return False

    def get_tensors(self):
        return self.x1, self.x2, self.x3, self.drop4
    
    def __call__(self, x1, x2, x3, drop4):
        del self.x1, self.x2, self.x3, self.drop4
        self.x1 = x1
        self.x2 = x2
        self.x3 = x3
        self.drop4 = drop4
...
class TimeDistributedDown(nn.Module):
    ...
    def forward(self, images):
        ...
        drop4, x1, x2, x3 = self.module(x_reshape)
        _, channels, height, width = drop4.size()
        # We have to reshape Y
        if self.batch_first:
            drop4 = drop4.contiguous().view(batch, timesteps, channels, height, width)  # (samples, timesteps, output_size)
        else:
            drop4 = drop4.view(timesteps, batch, channels, height, width)  # (timesteps, samples, output_size)

        return drop4, x1, x2, x3
...
class Sensor(nn.Module):
    ...
    def forward(self, x):
       ...
       # print(f'the outsize is {drop4.size()}')
        ...
        batch, timesteps, channels, height, width = drop4.size()
        drop4 = drop4.contiguous().view(batch*timesteps, channels, height, width) 

        out = self.tdUp(out, drop4, x1, x2, x3)
